...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function sanitizeZeroWidth(str) {
    ...
    // lineHeight が "normal" 等の文字列でも NaN にならない安全処理
    let parsedLineHeight = parseFloat(computedStyle.lineHeight);
    if (isNaN(parsedLineHeight) || parsedLineHeight <= 0) {
      const parsedFontSize = parseFloat(computedStyle.fontSize) || 16;
      parsedLineHeight = Math.round(parsedFontSize * 1.5); // 1.5倍を行高の安全値とする
    }
    lineMeasurer.style.lineHeight = parsedLineHeight + 'px';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const measuredH = lineMeasurer.offsetHeight;
      const h = (!measuredH || isNaN(measuredH) || measuredH < parsedLineHeight) 
        ? parsedLineHeight 
        : measuredH;

      lineNumHtml += `<div class="line-num-item" style="height: ${h}px; line-height: ${parsedLineHeight}px;">${i + 1}</div>`;
    }
